...
import { supabase, setAdminSecret } from "../utils/supabase";
...
export default function Dashboard() {
  ...
  const delayedProjects = projects.filter((p) => isProjectLate(p.extended_delivery_date || p.target_end_date)).length;
  const gracePeriodProjects = projects.filter((p) => {
    const today = new Date();
    const targetDate = new Date(p.extended_delivery_date || p.target_end_date);
